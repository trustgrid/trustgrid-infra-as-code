## Fixture: image_family empty string (negative / must fail)
## Story 5 — image selection behavior.
##
## image_family = "" is explicitly set.  The validation block rejects any
...
module "trustgrid_node" {
  ...
  ## Explicitly empty image_family — must be rejected.
  image_family = ""
}
